...
void init_decore(void);
void drawDecore(void);
...
bool check_building_collision(Vector2 position) {
    for(int i = 0; i < MAX_ELEMENTS; i++) {
        if(decore[i].element == 1) {
            Rectangle building_rect = {
                decore[i].coordinates.x - 5.0f, 
                (decore[i].coordinates.y - decore[i].height / 2) - 5.0f,
                decore[i].width + 10.0f, 
                decore[i].height + 10.0f
            };
            if(CheckCollisionPointRec(position, building_rect)) {
                return true;
            }
        }
    }
...
void push_out_of_building(Entity *entity) {
    if (!check_building_collision(entity->position)) return;
    Vector2 new_pos = entity->position;
    ...
    for(int i = 0; i < MAX_ELEMENTS; i++) {
        if(decore[i].element == 1) {
            ...
            Vector2 points[4] = {
                ...
                {building_rect.x, building_rect.y + building_rect.height},
                {building_rect.x + building_rect.width, building_rect.y + building_rect.height}
            };
            for (int j = 0; j < 4; j++) {
                float dist = distance(entity->position, points[j]);
                if (dist < closest_dist) {
                    closest_dist = dist;
                    new_pos = points[j];
                }
            }
            float mid_x = building_rect.x + building_rect.width / 2;
            float mid_y = building_rect.y + building_rect.height / 2;
            if (fabsf(entity->position.x - mid_x) < building_rect.width / 2) {
                if (entity->position.y < mid_y) {
                    new_pos.y = building_rect.y - 10.0f;
                } else {
                    new_pos.y = building_rect.y + building_rect.height + 10.0f;
                }
            }
            if (fabsf(entity->position.y - mid_y) < building_rect.height / 2) {
                if (entity->position.x < mid_x) {
                    new_pos.x = building_rect.x - 10.0f;
                } else {
                    new_pos.x = building_rect.x + building_rect.width + 10.0f;
                }
            }
        }
    }
...
void init_game() {
    ...
    for (int i = 0; i < 80; i++) {
        ...
        do {
            valid_position = true;
            pos = (Vector2){100 + (rand() % 600), 50 + (rand() % 620)};
            if (check_building_collision(pos)) {
    ...
    for (int i = 0; i < 59; i++) {
        ...
        do {
            valid_position = true;
            pos = (Vector2){680 + (rand() % 500), 50 + (rand() % 620)};
            if (check_building_collision(pos)) {
                valid_position = false;
            }
            attempts++;
    ...
    Vector2 heli_pos = {900 + (rand() % 300), 50 + (rand() % 620)};
...
void update_protester_ai(Entity *entity, int index) {
  ...
  Vector2 flocking = compute_flocking(entity, index, protesters, MAX_PROTESTERS);
    ...
    if (!collision) {
        collision = check_building_collision(new_pos);
    }
    if (!collision) {
        entity->position = new_pos;
...
void update_police_ai(Entity *entity, int index) {
    ...
    if (entity->police_type == HELICOPTER) {
        ...
        Vector2 new_pos = Vector2Add(entity->position, Vector2Scale(entity->velocity, GetFrameTime()));
        if (!check_building_collision(new_pos)) {
            entity->position = new_pos;
        }
...
void draw_projectiles() {
    for (int i = 0; i < MAX_PROJECTILES; i++) {
        if (projectiles[i].active) {
            DrawCircleV(projectiles[i].position, 3.0f, projectiles[i].type == PROTESTER ? BLACK : WHITE);
        }
    }
}
...
void init_decore(){
    ...
    for(int i=0;i<5;i++){
        if(index<MAX_ELEMENTS){
            ...
            decore[index].coordinates.y=50+(rand()%((SCREEN_HEIGHT)-200));
    ...
    for(int i=0;i<5;i++){
        if(index<MAX_ELEMENTS){
            decore[index].element=1;
            decore[index].coordinates.x=100+(rand()%((SCREEN_WIDTH)-300));
    ...
    for(int i=0;i<10;i++){
        if(index<MAX_ELEMENTS){
            ...
            while(!valid_position && attempts < 20) {
                if(i < 5) {
                    decore[index].coordinates.x = 30 + rand()%100;
                } else {
                    decore[index].coordinates.x = SCREEN_WIDTH - 130 + rand()%100;
                }
                decore[index].coordinates.y = 100 + rand()%(SCREEN_HEIGHT-200);
                
                valid_position = true;
                for(int j = 0; j < index; j++) {
                    if(decore[j].element == 1) {
                        float dist = distance(decore[index].coordinates, decore[j].coordinates);
                        if(dist < 150) {
                            valid_position = false;
                            break;
                        }
                ...
                for(int j = 0; j < MAX_BARRIERS; j++) {
                    if(barriers[j].active) {
                        Vector2 barrier_center = {
                            barriers[j].start.x, 
                            (barriers[j].start.y + barriers[j].end.y) / 2
                        };
                        float dist = distance(decore[index].coordinates, barrier_center);
                        if(dist < 100) {
                            valid_position = false;
                            break;
                        }
            ...
            decore[index].width = 20 + rand()%10;
            decore[index].height = 20 + rand()%10;
            index++;
        }
    }
}
    ...
    for(int i=0;i<MAX_ELEMENTS;i++){
        if(decore[i].element==1){
            Rectangle rect={
                ...
                decore[i].coordinates.y-decore[i].height/2, 
                decore[i].width, 
                decore[i].height
            };
            ...
            DrawCircle(decore[i].coordinates.x, decore[i].coordinates.y, 
                      decore[i].width/2, GREEN);
            DrawCircle(decore[i].coordinates.x, decore[i].coordinates.y, 
                      decore[i].width/4, BROWN);
        }
    }
}
